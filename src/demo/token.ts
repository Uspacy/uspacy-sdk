...
export const setToken = (token: string, expires = 7200) =>
	cookie.set(TOKEN_KEY, token, { expires: addSeconds(new Date(), expires) });
...
export const setRefreshToken = (token: string, expires = 365) =>
	cookie.set(REFRESH_TOKEN_KEY, token, {
		expires: addDays(new Date(), expires),
	});
export const getRefreshToken = (): string | undefined =>
	cookie.get(REFRESH_TOKEN_KEY);
